...
void usage(LPCWSTR exe) {
    LOG(
		...
		L"    --hostname send audio stream (up to 50 characters or %ls if omitted)\n"
		L"    --port send audio stream (%u if omitted)\n"
        L"    --int-16 attempts to coerce data to 16-bit integer format",
        exe, exe, exe, DEFAULT_HOST, DEFAULT_PORT
    );
}
...
: m_pMMDevice(NULL)
, m_hFile(NULL)
, m_bInt16(true) // Always true for legacy android os which does'nt support float PCM
, m_pwfx(NULL)
, m_szFilename(NULL)
, mPort(-1)
, hostname(NULL)
    ...
    switch (argc) {
        ...
        default:
            ...
            for (int i = 1; i < argc; i++) {
				...
				// --port
				if (0 == _wcsicmp(argv[i], L"--port")) {
					if (-1 != mPort) {
						ERR(L"%s", L"Only one --port switch is allowed");
						hr = E_INVALIDARG;
						return;
					}

					if (i++ == argc) {
						ERR(L"%s", L"--port switch requires an argument");
						hr = E_INVALIDARG;
						return;
					}

					mPort = wcstol(argv[i],NULL,10);//((char*)argv[i]);
					if (mPort <= 0){
						ERR(L"%s", L"Invalid port number");
						hr = E_INVALIDARG;
						return;
					}
				...
				// hostname
				if (0 == _wcsicmp(argv[i], L"--hostname")) {
					if (NULL != hostname) {
						ERR(L"%s", L"Only one --hostname switch is allowed");
						hr = E_INVALIDARG;
						return;
					}

					if (i++ == argc) {
						ERR(L"%s", L"--hostname switch requires an argument");
						hr = E_INVALIDARG;
						return;
					}
					...
					wcstombs_s(&ch_num, hostname, 50, argv[i], _TRUNCATE);
			...
			if (mPort == -1) {
				mPort = DEFAULT_PORT;
				LOG(L"Default port number will be used: %u", mPort);
			}

			// use default hostname
			if (hostname == NULL) {
				//hostname = "localhost";//DEFAULT_HOST;
				size_t ch_num;
				hostname = new char[10];
				wcstombs_s(&ch_num, hostname, 10, DEFAULT_HOST, _TRUNCATE);
				printf("Default hostname will be used: %s\n", hostname);
			}
