...
HRESULT LoopbackCapture(
    ...
    HRESULT hr;
	hFile = NULL; //just referring once to avoid warning
	...
	LOG(L"Mixformat:");
	LOG(L"	Format       :0x%08x", pwfx->wFormatTag);
	LOG(L"	BitsPersample:%u", pwfx->wBitsPerSample);
	LOG(L"	SamplesPerSec:%u", pwfx->nSamplesPerSec);
	//Set Audioformat to buffer
	AudioBuffer &audioBuffer = AudioBuffer::getInstance();
	audioBuffer.nChannels = pwfx->nChannels;
	audioBuffer.nSamplesPerSec = pwfx->nSamplesPerSec;
	audioBuffer.wBitsPerSample = pwfx->wBitsPerSample;
	char* ringbuffer = audioBuffer.initBuffer((int)(LONG)hnsDefaultDevicePeriod / (10 * 1000));
	if (ringbuffer == 0) {
		ERR("AudioBuffer initialization failed.");
		return E_UNEXPECTED;
	}
	deletePointer DelRingBuffer(ringbuffer);
	LPVOID abcs = audioBuffer.getCriticalSection();
	SetEvent(hWasapiReady);  //send ready signal
    ...
    *pnFrames = 0;
	...
	LOG(L"Buffersize:%u (frames)",BufferSize);
	...
	LOG("DevicePeriod:%lld (100 nanosec) lTimeBetweenFires:%ld (ms)", hnsDefaultDevicePeriod, lTimeBetweenFires);
    if (!bOK) {
        DWORD dwErr = GetLastError();
	...
	DWORD dwWaitResult = WaitForSingleObject(hSocketReady, INFINITE);
	if (WAIT_OBJECT_0 != dwWaitResult) {
		ERR(L"WaitForSingleObject(hSocketReady) returned unexpected result 0x%08x, last error is %d", dwWaitResult, GetLastError());
		return E_UNEXPECTED;
	}
	...
	bool _SILENT = false;
	bool SILENT = false;
	UINT32 inSilentPass = 0;
	UINT32 outSilentPass = 0;
    for (UINT32 nPasses = 0; !bDone; nPasses++) {
        // drain data while it is available
        UINT32 nNextPacketSize;
		int GetBufLoop = 0;
		//bool getbufExecuted = false;
        ...
        for (
			...
			_SILENT = SILENT;
			//getbufExecuted = true;
			//printf("get buffer executed. nNextPacketSize = %u \n", nNextPacketSize);
			...
			//if (bFirstPacket && AUDCLNT_BUFFERFLAGS_SILENT == dwFlags) {
				...
				SILENT = true;
				if (_SILENT == false) {
					LOG(L"get into silent. GetBufLoop = %d nNumFramesToRead = %u pass:%u", GetBufLoop,nNumFramesToRead, nPasses);
					inSilentPass = nPasses;
					bFirstPass = true;
					/*LONG lBytesToWrite = nNumFramesToRead * nBlockAlign;
					printf("Silent Packet:");
			...
			if (_SILENT == true && SILENT == false) {
				outSilentPass = nPasses;
				LOG(L"get out of silent. GetBufLoop:%d nNumFramesToRead = %u pass:%u Silent Duration(pass):%u", GetBufLoop, nNumFramesToRead, nPasses,(outSilentPass - inSilentPass));
				bFirstPass = true;
				/*LONG lBytesToWrite = nNumFramesToRead * nBlockAlign;
				printf("Not Silent Packet:");
				for (LONG i = 0; i < lBytesToWrite; i++) {
				printf("%u,",*(pData+i));
				}
				printf("\n");*/
			...
			if (bFirstPass) {
				LOG(L"(Pass:%u)NextPacketSize:%u (frames) GetBufLoop:%d", nPasses, nNextPacketSize, GetBufLoop);
			}
			//if (SILENT == false) {
				LONG lBytesToWrite = nNumFramesToRead * nBlockAlign;
#pragma prefast(suppress: __WARNING_INCORRECT_ANNOTATION, "IAudioCaptureClient::GetBuffer SAL annotation implies a 1-byte buffer")
			...
			//}
				...
				int n = audioBuffer.write(reinterpret_cast<char*>(pData), lBytesToWrite);
				if (n == 1) {
					ERR(L"AudioBuffer write error.");
					return E_UNEXPECTED;
				}
				LeaveCriticalSection((LPCRITICAL_SECTION)abcs);
				SetEvent(hWasapiReady);
				//debug
				/*
				...
				long readsize;
				n = audioBuffer.read(buffer, &readsize, buffersize);
				if (n == 1) {
					printf("Reading from audiobuffer failed.");
					return 1;
				}
				LONG lBytesWritten = mmioWrite(hFile, buffer, readsize);
				if (readsize != lBytesWritten) {
				ERR(L"mmioWrite wrote %u bytes on pass %u after %u frames: expected %u bytes", lBytesWritten, nPasses, *pnFrames, lBytesToWrite);
				return E_UNEXPECTED;
				}
				*/
		...
		//if (!getbufExecuted)printf("get buffer not executed.\n");
		...
		if(!bFirstPacket)bFirstPass = false;
    } // capture loop
